...
if [[ -f "$WORK_KEY_PUB" || -L "$WORK_KEY_PUB" ]]; then
  ...
  ssh-keygen -t rsa -b 4096 -f /home/$USER_NAME/.ssh/nentgroup_rsa -C "victor.conner@nentgroup.com"
fi
...
if [[ -f "$PERSONAL_KEY_PUB" || -L "$PERSONAL_KEY_PUB" ]]; then
  ...
  ssh-keygen -t rsa -b 4096 -f /home/$USER_NAME/.ssh/private_rsa -C "charlesvictorconner@gmail.com"
fi
