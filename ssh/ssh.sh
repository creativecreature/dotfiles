...
WORK_KEY_PUB=/home/$USER/.ssh/nentgroup_rsa.pub
PERSONAL_KEY_PUB=/home/$USER/.ssh/private_rsa.pub
...
if [[ -f "$WORK_KEY_PUB" || -L "$WORK_KEY_PUB" ]]; then
  ...
  ssh-keygen -t rsa -b 4096 -f /home/$USER/.ssh/nentgroup_rsa -C "victor.conner@nentgroup.com"
fi
...
if [[ -f "$PERSONAL_KEY_PUB" || -L "$PERSONAL_KEY_PUB" ]]; then
  ...
  ssh-keygen -t rsa -b 4096 -f /home/$USER/.ssh/private_rsa -C "charlesvictorconner@gmail.com"
fi

echo_item "Symlinking config to ~/.ssh/config file" "green"
ln -sf /home/$USER/dotfiles/ssh/config /home/$USER/.ssh/config
